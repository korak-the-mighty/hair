...
function tts(voiceId, text, style) {
  ...
  try {
    const buf = call('POST', url, { text: st.tag + text, model_id: 'eleven_v3', voice_settings: { stability: 0.5, similarity_boost: st.settings.similarity_boost } }, 'audio/mpeg');
...
function main() {
  ...
  for (const j of jobs) {
    ...
    const buf = j.kind === 'hook' ? tts(voices[j.voice], j.text, j.style) : chop(j, voices);
