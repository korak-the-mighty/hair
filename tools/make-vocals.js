 ...
 * robot lines are plain, steady speech that the page's vocoder sings.
...
function main() {
  ...
  for (const v of spec.robot || []) {
    ...
    if (fs.existsSync(f)) clips.push({ id: v.id, kind: 'robot', text: v.text, voice: v.voice, data: fs.readFileSync(f).toString('base64') });
  }
